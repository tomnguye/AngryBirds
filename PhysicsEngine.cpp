...
void PhysicsEngine::advanceState() {
    ...
    struct ContactImpulseAccumulator {
        float normal = 0.f;
        float friction = 0.f;
    };
    int totalContacts = 0;
...

    {
        ...
        for (auto &event : c) {
            ...
            for (int i = 0; i < (int)event.contactPoints.size(); i++, ci++) {
                ...
                Eigen::Vector2f vA =
                    a->velocity + Eigen::Vector2f{-a->angularVelocity * rA.y(),
                                                  a->angularVelocity * rA.x()};
                Eigen::Vector2f vB =
                    b->velocity + Eigen::Vector2f{-b->angularVelocity * rB.y(),
    ...
    for (int iteration = 0; iteration < maxIterations; iteration++) {
        ...
        for (auto &event : c) {
            ...
            for (int i = 0; i < (int)event.contactPoints.size(); i++, ci++) {
                ...
                a->angularVelocity -=
                    (rA.x() * Jn.y() - rA.y() * Jn.x()) * a->inv_inertia;
                b->angularVelocity +=
                    (rB.x() * Jn.y() - rB.y() * Jn.x()) * b->inv_inertia;
                ...
                accumulatedImpulses[ci].friction =
                    std::clamp(jtOld + jt, -coulombLimit, coulombLimit);
                ...
                a->angularVelocity -=
                    (rA.x() * Jf.y() - rA.y() * Jf.x()) * a->inv_inertia;
                b->angularVelocity +=
                    (rB.x() * Jf.y() - rB.y() * Jf.x()) * b->inv_inertia;
            }
        }
    }
    ...
    for (auto &obj : objects) {
        ...
        if (obj->sleeping)
            continue;
        if (obj->velocity.norm() < 0.015f &&
            fabsf(obj->angularVelocity) < 0.015f) {
            obj->sleepTimer++;
        ...
        //     if (fabsf(obj->rotation - rounded) < 0.01f) obj->rotation =
        //     rounded;
        // }
    }
}

...
public:
    ...
    PhysicsObject(obj_type t, float i,
                  ...
                  Eigen::Vector2f v = Eigen::Vector2f::Zero(), float r = 0,
                  float w = 0, bool s = false, float m = 1) {
        type = t;
        position = p;
        velocity = v;
        rotation = r;
...
public:
    float radius;
    Circle(float r, Eigen::Vector2f p = Eigen::Vector2f::Zero(),
           Eigen::Vector2f v = Eigen::Vector2f::Zero(), float rot = 0,
           float w = 0, bool s = false, float m = 10)
...
public:
    ...
    float baseAngle;
    Rectangle(Eigen::Vector2f d, Eigen::Vector2f p = Eigen::Vector2f::Zero(),
              Eigen::Vector2f v = Eigen::Vector2f::Zero(), float r = 0,
              float w = 0, bool s = false, float m = 10,
              bool isBoundary = false)
        : PhysicsObject(RECTANGLE,
                        (1.0f / 12.0f) * m * (d[0] * d[0] + d[1] * d[1]), p, v,
                        r, w, s, m) {
        dimentions = d;
